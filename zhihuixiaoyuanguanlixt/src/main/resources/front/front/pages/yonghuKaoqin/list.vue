	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 0px 0px rgba(0, 0, 0, 0.16)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"0 180rpx 180rpx 0","borderWidth":"4rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}'
						v-model="searchForm.yonghuKaoqinName" type="text" placeholder="考勤标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('yonghuKaoqin','新增')" @click="onAddTap()"
        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <h4>{{ item.yonghuKaoqinName }}</h4>
                    <view>
                        <uni-icons v-show="isAuth('yonghuKaoqin','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
                <!-- 当前表 -->
                 <view v-show="item.yonghuKaoqinUuidNumber" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">考勤唯一编号</text>
                     <text style="width: 70%">
                         <text v-if="item.yonghuKaoqinUuidNumber != null">{{item.yonghuKaoqinUuidNumber}}</text>
                         <text v-else></text>
                     </text>
                 </view>
                <view v-show="item.yonghuKaoqinContent" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">考勤详情</text>
                    <view style="width: 70%" v-html="item.yonghuKaoqinContent"></view>
                </view>
                <view v-show="item.jiezhiTime" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                    <text style="width: 25%;margin-right: 5%;">考勤截止时间</text>
                    <text style="width: 70%">
                        <text v-if="item.jiezhiTime != null">{{item.jiezhiTime}}</text>
                        <text v-else></text>
                    </text>
                </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-show="isAuth('yonghuKaoqin','修改')" @tap="onUpdateTap(item.id)" type="compose" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('yonghuKaoqin','审核') && item.yonghuKaoqinYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`yonghuKaoqin`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yonghuKaoqin', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.yonghuKaoqinShenheTime=shijian;
                            await _this.$api.update(`yonghuKaoqin`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.yonghuKaoqinUuidNumber) {
                    searchForm['yonghuKaoqinUuidNumber'] = this.searchForm.yonghuKaoqinUuidNumber
                }
				if (this.searchForm.yonghuKaoqinName) {
                    searchForm['yonghuKaoqinName'] = this.searchForm.yonghuKaoqinName
                }

                let res = await this.$api.page(`yonghuKaoqin`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.yonghuKaoqinContent!=null){
                        item.yonghuKaoqinContent = item.yonghuKaoqinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
