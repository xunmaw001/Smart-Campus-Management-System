	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yonghuKaoqinId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yonghuKaoqin', this.yonghuKaoqinId);
                    this.detail = res.data
					this.detail.yonghuKaoqinContent = this.detail.yonghuKaoqinContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.yonghuKaoqinPhoto ? this.detail.yonghuKaoqinPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yonghuKaoqinYuyue', {
									yonghuId: _this.user.id,
									yonghuKaoqinId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yonghuKaoqinYuyue/list');
							}
						}
					});
				},
