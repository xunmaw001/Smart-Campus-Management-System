        ...
        <view style="padding: 0 20rpx;width: 100%;margin: 0 auto;;flex-direction: column;flex-wrap: wrap;flex-direction: row;margin-top: 0upx;">
            <view class="list b-b" v-for="(item, index) in list" :key="index" style="width:100%;height: auto;border: 1rps solid #999;margin: 0 auto;background-color: #FFFFFF;margin-top: 10rpx; box-shadow: 0  0 10rpx 0 #999999;border-radius: 25rpx;">
                <view
                    ...
                    <view>
                        <uni-icons v-show="isAuth('yonghuKaoqinList','删除')" @tap="onDeleteTap(item.id)" type="closeempty"
                                   size="20"></uni-icons>
                    </view>
                </view>
                <!-- 级联表 -->
				<view v-show="item.yonghuName" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
				    <text style="width: 25%;margin-right: 5%;">考勤结束时间</text>
                ...
                <!-- 当前表 -->
                 <view v-show="item.yonghuKaoqinListValue" style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;">
                     <text style="width: 25%;margin-right: 5%;">打卡状态</text>
                     <text style="width: 70%">{{item.yonghuKaoqinListValue}}</text>
                 </view>
            <view style="display: flex;justify-content: space-between;align-items: center;margin-top: 15rpx;font-size: 24rpx;color: #8799A3;">
                ...
                <uni-icons v-if="item.yonghuKaoqinListTypes==1" @tap="yonghuKaoqinListDaka(item.id)" type="checkmarkempty" size="20">
                </uni-icons>
                <!--<view v-if="isAuth('yonghuKaoqinList','审核') && item.yonghuKaoqinListYesnoTypes==1" class="" style="display: flex; width: 70%;color: #FFFFFF;">
	...
	export default {
		...
		methods: {
			...
			async upCallback(mescroll) {
                ...
                let _this = this
                let res = await this.$api.page(`yonghuKaoqinList`, params);
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							let data1 = await _this.$api.info('yonghuKaoqinList', id);
							let datadata1= data1.data;
							if(datadata1==null){
							    _this.$utils.msg("查不到打卡信息,请核实");
							    return false;
							}else if(datadata1.yonghuKaoqinListTypes ==2){
							    _this.$utils.msg("已经打过卡了,不能重复打卡");
							    return false;
							}else{
								let data2 = await _this.$api.info('yonghuKaoqin', datadata1.yonghuKaoqinId);
								if(data2.data==null){
									_this.$utils.msg("被打卡表查询不到信息");
									return false;
								}else{
									var jiezhiTime=new Date(data2.data.jiezhiTime.replace(/-/g,"/"));
									if(jiezhiTime.getTime()< new Date().getTime()){
										_this.$utils.msg("超过打卡时间了,不能打卡了");
										return false;
									}
									datadata1.yonghuKaoqinListTypes=2;
									//更新打卡状态并添加打卡时间
									await _this.$api.update('yonghuKaoqinList', datadata1);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('yonghuKaoqinList', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
							_this.mescroll.resetUpScroll()
						}
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.yonghuKaoqinListShenheTime=shijian;
                            await _this.$api.update(`yonghuKaoqinList`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
                ...
                let res = await this.$api.page(`yonghuKaoqinList`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                })
				// 如果是第一页数据置空
