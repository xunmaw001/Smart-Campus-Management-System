	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yonghuKaoqinListId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yonghuKaoqinList', this.yonghuKaoqinListId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yonghuKaoqinListPhoto ? this.detail.yonghuKaoqinListPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yonghuKaoqinListYuyue', {
									yonghuId: _this.user.id,
									yonghuKaoqinListId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yonghuKaoqinListYuyue/list');
							}
						}
					});
				},
