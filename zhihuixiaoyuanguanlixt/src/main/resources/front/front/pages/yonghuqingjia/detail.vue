	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.yonghuqingjiaId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('yonghuqingjia', this.yonghuqingjiaId);
                    this.detail = res.data
					// 主图
					this.swiperList = this.detail.yonghuqingjiaPhoto ? this.detail.yonghuqingjiaPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('yonghuqingjiaYuyue', {
									yonghuId: _this.user.id,
									yonghuqingjiaId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/yonghuqingjiaYuyue/list');
							}
						}
					});
				},
