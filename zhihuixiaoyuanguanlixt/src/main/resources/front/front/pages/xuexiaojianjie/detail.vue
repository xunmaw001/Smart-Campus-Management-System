<template>
			<view style="margin: 0 30rpx;box-shadow:0 0px 0px 1rpx #a6a6a6">
				<view class="title">
					{{detail.xuexiaojianjieName}}
				</view>
				<view style="font-weight: bold;font-size: 20upx;margin:0 30rpx 5rpx;"  :style="(detail.xuexiaojianjieValue == null || detail.insertTime == null)?'text-align: center;':''">
					<text v-if="detail.xuexiaojianjieValue">
						{{detail.xuexiaojianjieValue}}
					</text>
					<text :style="detail.xuexiaojianjieValue?'float: right':''">
						{{detail.insertTime}}<!-- .split(' ')[0] -->
					</text>
				</view>
				<view style="box-shadow: 0 0  0 1rpx #a6a6a6;margin: 0 30rpx"></view>
                <viwe>
					<image :src="baseUrl+detail.xuexiaojianjiePhoto"></image>
                </viwe>
				<view class="content">
					<rich-text :nodes="detail.xuexiaojianjieContent"></rich-text>
				</view>
			</view>
	...
	<script>
		export default {
			...
			async onLoad(options) {
				this.xuexiaojianjieId = options.id;
			},
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('xuexiaojianjie', this.xuexiaojianjieId);
                    this.detail = res.data
					this.detail.xuexiaojianjieContent = this.detail.xuexiaojianjieContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.xuexiaojianjiePhoto ? this.detail.xuexiaojianjiePhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('xuexiaojianjieYuyue', {
									yonghuId: _this.user.id,
									xuexiaojianjieId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/xuexiaojianjieYuyue/list');
							}
						}
					});
				},
