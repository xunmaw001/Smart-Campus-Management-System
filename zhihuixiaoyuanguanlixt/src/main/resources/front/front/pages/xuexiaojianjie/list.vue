	...
	<view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
        <view class="search-form round">
				<input :style='{"padding":"0 24rpx","boxShadow":"0px 0px 0px rgba(0, 0, 0, 0.16)","margin":"0","borderColor":"var(--publicMainColor)","backgroundColor":"#fff","color":"#333","textAlign":"left","borderRadius":"0 180rpx 180rpx 0","borderWidth":"4rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}'
						v-model="searchForm.xuexiaojianjieName" type="text" placeholder="标题"></input>
		</view>
        ...
        <view class="action">
            ...
            <view v-if="isAuth('xuexiaojianjie','新增')" @click="onAddTap()"
            ...
            <uni-drawer ref="showRight" mode="left" :maskClick="true" :width="100" :mask-click="true">
                <scroll-view scroll-y="true" class="tabView" :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"10rpx 0 0 ","borderColor":"rgba(0,0,0,1)","borderRadius":"0","borderWidth":"0","width":"150rpx","borderStyle":"solid","height":"auto"}'>
                    <view style="display: flex;flex-direction: column;">
                        <view :style='xuexiaojianjieTypes===item.codeIndex?sactiveItem:sitem' class="tab"
                              v-for="(item,index) in xuexiaojianjieTypesList" :key="index"
                              :class="xuexiaojianjieTypes===item.codeIndex?'tabActive':''"
                              @tap="xuexiaojianjieTypesCategoryClick(item.codeIndex)">{{item.indexName}}</view>
                    </view>
                </scroll-view>
            </uni-drawer>
<view class="news-box3"
    ...
    <view @tap="onDetailTap(item)" v-for="(item,index) in list" :key="index" class="list-item"
        ...
        <view
                ...
                class="title hide1">{{item.xuexiaojianjieName}}
        </view>
	...
	export default {
		...
		async onShow() {
            ...
            _this.btnColor = _this.btnColor.sort(() => {
                ...
                this.xuexiaojianjieTypesSelect()
		...
		methods: {
            ...
            //当前表的 简介类型 字段 字典表查询方法
            async xuexiaojianjieTypesSelect(){
                let xuexiaojianjieTypes = await this.$api.page('dictionary', {
                    page: 1,
                    limit: 100,
                    dicCode:'xuexiaojianjie_types',
                });
                xuexiaojianjieTypes.data.list.splice(0, 0, {codeIndex: 0, indexName: '全部'})
                this.xuexiaojianjieTypesList = xuexiaojianjieTypes.data.list
            },
            //当前表的 简介类型字段类别搜索
			xuexiaojianjieTypesCategoryClick(categoryName) {
			...
			async upCallback(mescroll) {
                ...
                if (this.xuexiaojianjieTypes != 0) {
                    params.xuexiaojianjieTypes = this.xuexiaojianjieTypes
                }
                let _this = this
                let res = await this.$api.list(`xuexiaojianjie`, params);
			...
			onDeleteTap(id) {
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('xuexiaojianjie', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
                ...
                uni.showModal({
                    ...
                    success: async function(res) {
                        if (res.confirm) {
                            ...
                            data.xuexiaojianjieShenheTime=shijian;
                            await _this.$api.update(`xuexiaojianjie`, data);
                            _this.hasNext = true
                            // 重置分页参数页数为1
                            _this.mescroll.resetUpScroll()
                        }
			...
			async search() {
				...
				if (this.searchForm.xuexiaojianjieName) {
                    searchForm['xuexiaojianjieName'] = this.searchForm.xuexiaojianjieName
                }

                let res = await this.$api.list(`xuexiaojianjie`, searchForm);
                let _this = this
                res.data.list.forEach(function(item,index){
                    if(item.xuexiaojianjieContent!=null){
                        item.xuexiaojianjieContent = item.xuexiaojianjieContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+_this.$base.url+"upload/");
