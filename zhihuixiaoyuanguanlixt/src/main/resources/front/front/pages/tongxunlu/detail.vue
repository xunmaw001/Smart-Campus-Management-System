	...
	<script>
		export default {
			...
			methods: {
				...
				// 获取详情
				async init() {
					let res = await this.$api.info('tongxunlu', this.tongxunluId);
                    this.detail = res.data
					this.detail.tongxunluContent = this.detail.tongxunluContent.replace("<img","<img  class=\"daxiao\"").replace("src=\"upload/","src=\""+this.$base.url+"upload/");
					// 主图
					this.swiperList = this.detail.tongxunluPhoto ? this.detail.tongxunluPhoto.split(",") : [];
				},
				...
				async baoming() {
					...
					uni.showModal({
						...
						success: async function(res) {
							if (res.confirm) {
								await _this.$api.add('tongxunluYuyue', {
									yonghuId: _this.user.id,
									tongxunluId: _this.detail.id,
								});
								_this.$utils.msg('报名成功');
								_this.$utils.jump('/pages/tongxunluYuyue/list');
							}
						}
					});
				},
